...
subtitle: "Un estudio longitudinal entre indígenas y no indígenas en Chile (2016 - 2024) "
authors: "Matías Deneken & Eduardo Valenzuela (editores)"
institute: "Matías Deneken & Eduardo Valenzuela (Editores)"
date: today
format: 
  revealjs:
    theme: simple
...
# ===============================================================================
# BLOQUE LISTO · SIN DISTINCIÓN PARA "e*" · (SIN INFO DE ESCALAS EN LA GRÁFICA)
# Subtítulo: “Cruce por pertenencia: Indígena vs No indígena”
# ===============================================================================

# ===============================================================================
# BLOQUE LISTO (SIN INFO DE ESCALAS / TOP-BOX EN LA SALIDA)
# Subtítulo estándar: “Cruce por pertenencia: Indígena vs No indígena”
# ===============================================================================
...
# --- 1) Helper: invertir escala 1–5  (1↔5, 2↔4, 3→3) ---------------------------
...
# --- 2) Aplicar inversión EN SITIO a a2_* ---------------------------------------
panel_data <- panel_data %>%
  dplyr::mutate(across(dplyr::all_of(a2_vars), invertir_1a5))

# 🔒 Tamaño global fijo p/ slides -------------------------------------------------
...
# 8) Detectar variables "e" (bloque común) ---------------------------------------
e_all <- names(panel_data)[grepl("^e\\d+_", names(panel_data))]

# 9) Temas (E en un único bloque) ------------------------------------------------
...
# 12) Porcentaje "alto" (interno; sin exponer reglas en la gráfica) --------------
...
# 17) LÍNEAS: varias variables (facetas) -----------------------------------------
...
> <br> [La presentación se organiza en cuatro módulos comparativos que permiten observar, a lo largo del tiempo, las diferencias y convergencias entre personas indígenas y no indígenas.]{style="font-size:0.7em; color:#556B2F; font-style:italic;"}
> :::
...
plot_variables_por_indigeneidad_line(c("a4","a6"),
                                     titulo = "Identificación en términos comparados", ci = FALSE)
```
...
caption_extra <- if (length(olas_no) > 0) {
  paste0(" · No se preguntó en ", paste(olas_no, collapse = ", "), ".")
} else { "" }
...
if (nrow(tendencias) == 0) {
  ...
  # --- Graficar (misma estética que tu función) ---
  ggplot(tendencias_plot, aes(x = fecha_ola, y = porcentaje, fill = grupo)) +
    ...
    labs(
      ...
      x = "Olas de seguimiento", y = "Porcentaje (0–100)",
      caption = cap
    ) +
    theme_esep_longitudinal()
}
...
plot_variables_por_indigeneidad_line(c("c17_2","c17_4"),
                                     titulo = "Empatía intergrupal", ci = FALSE)
```
